...
from Cosmo212.analysis import analysis_2d, analysis_mid
...
fname_for_saving='Results_JLA/FW3_20kfor3params/DPlots/'
...
def proposal_covariance(choice):
    if (choice=='FL2') or (choice=='OL2'):
        sig=0.02
        diag=0.02**2/5
        offdiag=-diag/2
        covmat=(np.array([[diag, offdiag], [offdiag, diag]]).reshape(2, 2))*10
        if choice=='OL2':
            covmat=10*covmat
        return covmat
    if (choice == 'FW2'):
    ...
    if (choice=='FW3'):
        diag1, diag2, diag3 = 0.02 ** 2, 0.07 ** 2, 0.02**2
        offdiag1, offdiag2, offdiag3 = -diag1 / 2, (-diag1 / 2), (diag1 / 2)
        covmat = np.array([[diag1, offdiag1, offdiag2], [offdiag1, diag2, offdiag3], [offdiag2, offdiag3, diag3]]).reshape(3, 3)
        return ((2.4) ** 2.0) * covmat * 0.5/3.0
    if (choice=='OL3'):
        diag1, diag2, diag3 = 0.02 ** 2, 0.02 ** 2, 0.02**2
        offdiag1, offdiag2, offdiag3 = -diag1 / 2, (-diag1 / 2), (diag1 / 2)
        covmat = np.array([[diag1, offdiag1, offdiag2], [offdiag1, diag2, offdiag3], [offdiag2, offdiag3, diag3]]).reshape(3, 3)
        return ((2.4) ** 2.0) * covmat * 3.0
...
def mcmc_per_chain(ch, theta_start, choice, covar_t, fname_prefix, mu_obs, cov_inv, z_arr):
    ...
    for iter in range(1, Num_iter):
        ...
        if (ch==0) & (iter==2000): #to check efficiency post 20k steps, comment out all non-efficiency funcs in analysis if called
            analysis_mid(fname_prefix+'chcont_', fname_for_saving, 4, choice)
...
if __name__=='__main__':
    ...
    analysis_2d(fname_for_saving+'chfinal_', fname_for_saving+'FW3/', 4, model) #make sure you a) uncomment above and b) check whether that code has been adjusted for jla
